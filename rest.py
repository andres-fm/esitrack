...
class alphabetResource(RESTResource):
    def handle_GET(self, *vpath, **params):
        if len(vpath) == 3:
            if vpath[1] == 'list':
                return str(cherrypy.session[vpath[2]].words)
            elif vpath[1] == 'view':
                return str(cherrypy.session[vpath[2]])
        raise Exception("ni list ni view en GET")
    ...
    def handle_PUT(self, *vpath, **params):
        if vpath[0] == 'alphabetSoup':
            ...
            return str(soup.encuentra(params['sr'], params['sc'], params['er'], params['ec']))
